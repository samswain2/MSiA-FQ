...
#### Explainable model
```{r}
hand_reduced_model = lm(trips ~ PARK_AREA_ACRES + CAPACITY + CTA_TRAIN_STATIONS + MINORITY + ARSON + ASSAULT*BATTERY + CONCEALED_CARRY_LICENSE_VIOLATIO + HOMICIDE + STALKING, data = df)

summary(hand_reduced_model)
vif(hand_reduced_model, type = 'predictor')
```
...
aic_model = lm(trips ~ (.)^2, df_selected)

aic_step_model <- stepAIC(aic_model, 
                      direction = "both", 
                      trace = FALSE)

summary(aic_step_model)
vif(aic_step_model, type = 'predictor')
```